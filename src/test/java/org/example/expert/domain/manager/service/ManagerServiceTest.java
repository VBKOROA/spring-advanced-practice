...
class ManagerServiceTest {
    ...
    class 담당자_지정_테스트 {
        ...
        void 조회된_Todo가_없으면_실패한다() {
            ...
            assertThrows(InvalidRequestException.class,
                    () -> managerService.saveManager(authUser, 1L, null));
        }
        ...
        @Test // 테스트코드 샘플
        void 담당자로_지정할_User가_조회되지않으면_실패한다() {
            // given
            AuthUser authUser = new AuthUser(1L, "a@a.com", UserRole.USER);
            User user = User.fromAuthUser(authUser); // 일정을 만든 유저
    ...
    class Todo_담당자_삭제_테스트 {
        ...
        @Test
        void Todo를_조회하지_못하면_실패한다() {
            // given
            User todoCreator = new User("null", "null", UserRole.USER);

            given(userRepository.findById(anyLong())).willReturn(Optional.of(todoCreator));
            given(todoRepository.findById(anyLong())).willReturn(Optional.empty());
